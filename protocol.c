...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_WDJC, DP_TYPE_VALUE},
  {DPID_JRSC, DP_TYPE_BOOL},
  {DPID_ZLSC, DP_TYPE_BOOL},
  {DPID_SDWD, DP_TYPE_VALUE},
  {DPID_DLFX, DP_TYPE_BOOL},
};
...
void all_data_update(void)
{
  ...
  // This code is automatically generated for the platform. Please modify each function according to the actual data
    mcu_dp_value_update(DPID_FREQUENCY,当前频率); //VALUE型数据上报;
    mcu_dp_value_update(DPID_VOLTAGE,当前电池电量); //VALUE型数据上报;
    mcu_dp_value_update(DPID_WDJC,当前检测温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_JRSC,当前加热输出); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_ZLSC,当前制冷输出); //BOOL型数据上报;
    mcu_dp_value_update(DPID_SDWD,当前设定温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_DLFX,当前电流方向); //BOOL型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_jrsc_handle
功能描述 : 针对DPID_JRSC的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_jrsc_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    jrsc = mcu_get_dp_download_bool(value,length);
    if(jrsc == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_JRSC,jrsc);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_zlsc_handle
功能描述 : 针对DPID_ZLSC的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_zlsc_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    zlsc = mcu_get_dp_download_bool(value,length);
    if(zlsc == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_ZLSC,zlsc);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_sdwd_handle
功能描述 : 针对DPID_SDWD的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sdwd_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long sdwd;
    
    sdwd = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_SDWD,sdwd);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_dlfx_handle
功能描述 : 针对DPID_DLFX的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_dlfx_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    dlfx = mcu_get_dp_download_bool(value,length);
    if(dlfx == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_DLFX,dlfx);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  switch(dpid)
  {
        ...
        case DPID_JRSC:
            //加热输出处理函数
            ret = dp_download_jrsc_handle(value,length);
        break;
        case DPID_ZLSC:
            //制冷输出处理函数
            ret = dp_download_zlsc_handle(value,length);
        break;
        case DPID_SDWD:
            //设定温度处理函数
            ret = dp_download_sdwd_handle(value,length);
        break;
        case DPID_DLFX:
            //电流方向处理函数
            ret = dp_download_dlfx_handle(value,length);
        break;
